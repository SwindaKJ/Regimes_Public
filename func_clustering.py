...
def distance( data, cluster ):
    ...
    dist = np.linalg.norm( diff, axis=(1,2) )       # Compute the norm of difference
    dist = dist /(data.shape[1]*data.shape[2])      # Normalise
...
def kmeans( data, k_nr, tol ):
    ...
    theta = np.random.randn( k_nr, lat_nr, lon_nr ) * np.amax( data )
...
def min_theta( data, k_nr, tol, gam, theta_init ):
    ...
    while ( j < step_max and dist_max > tol ):
        ...
        for k in range(0,k_nr):                                 # Set the range
            dist[k] = distance( data, theta[k] )                # Compute norm
...
def kmeans_pers( data, k_nr, c_nr, tol ):

    # Get time of dataset
    time_nr = data.shape[0]
    lon_nr = data.shape[2]
    lat_nr = data.shape[1]
    ...
    "The clusters are k latitude x longitude (25x49) matrices."
    theta_init = np.random.rand( k_nr, lat_nr, lon_nr ) * np.amax(data)
    ...
    theta, L, dist = min_theta( data, k_nr, tol, gam, theta_init )
    ...
    while ( j < step_max and abs( L - L0 ) > tol ):
        ...
        gam = min_gamma( dist, c_nr )

        # Compute the next clusters for the new weights
        theta, L, dist = min_theta( data, k_nr, tol, gam, theta )
        print(L)
